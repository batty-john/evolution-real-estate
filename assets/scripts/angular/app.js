var evo = angular.module('evo', []);

evo.controller('NgPropertiesController', ['$scope', '$http', function($scope, $http) {
    $scope.listBusy = false;
    $scope.offset = 0;
	$scope.listings = [];
	$scope.filter = {};
    $scope.loadListings = function(incrementValue) {
        loadListings(incrementValue);
    };
    function loadListings(incrementValue) {
		debugger;
		$scope.query = {};

        $scope.listBusy = true;
        $scope.showLoading = true;
        if($scope.offset + incrementValue <= 0) $scope.offset = 0;
        else if(!incrementValue) $scope.offset = 0;
		else $scope.offset += incrementValue;
		debugger;
		var extra = [];
		if($scope.filter.minSqFt) extra.push('&minSqFt=' + $scope.filter.minSqFt);
		if($scope.filter.maxSqFt) extra.push('&maxSqFt=' + $scope.filter.maxSqFt);
		if($scope.filter.minPrice) extra.push('&minPrice=' + $scope.filter.minPrice);
		if($scope.filter.maxPrice) extra.push('&maxPrice=' + $scope.filter.maxPrice);
		if($scope.filter.addressCityZipMls) extra.push('&addressCityZipMls=' + $scope.filter.addressCityZipMls);
		if($scope.category) extra.push('&category=' + $scope.category);
		var extraParams = extra.length > 0 ? extra.join('') : '';

        var url = '/properties/get?' + 'offset=' + $scope.offset + extraParams;
        $http.get(url).then(function(data) {
            debugger;
            var newListings = data.data;
            if(newListings && newListings.length > 0) {
                _.each(newListings, function(l) {
                    l.dateModified = moment(l.dtlastmod).fromNow();
                    $scope.listings.push(l);
                });
            }
			$scope.listBusy = false;
			$scope.$emit('listingsReloaded');
        }, function(err) {
            $scope.listBusy = false;
		});
	}

	$scope.assignCategory = function(category, optional) {
		debugger;
		if(optional === 'search') {
			$scope.filter.addressCityZipMls = category;
		} else {
	...
	$scope.reQueryWithNewFilter = function() {
		$scope.offset = 0;
		$scope.listings = [];
		loadListings();
	};
...
// FEATURED
evo.controller('NgFeaturedController', ['$scope', '$http', function($scope, $http) {
    $scope.listBusy = false;
    $scope.offset = 0;
	$scope.listings = [];
	$scope.filter = {};
...
evo.controller('NgListingController', ['$scope', '$http', function($scope, $http) {
	$scope.saveListing = function() {
		
	}
}]);

evo.directive('listingMaps', function() {
	return {
		link: function(scope) {
			(function($) {

				scope.$on('listingsReloaded', function() {
					debugger;
					console.log(scope.listings);
					mainMap(scope.listings);
					singlePropertyMap();
				});
				...
				//   ];
				  ...
				  var map = new google.maps.Map(document.getElementById('map'), {
					...
					center: new google.maps.LatLng(40.2172, -110.1),
